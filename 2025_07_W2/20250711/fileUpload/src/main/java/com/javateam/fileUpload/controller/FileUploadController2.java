...
public class FileUploadController2 {
	...
	// application.properties 파일의 imageUpload.path=E:/students/lsh/upload/image/
	private String imageUploadPath; 
	
	@Autowired
	FileNamingEnDecodingUtil fileNamingEnDecodingUtil;
	
	@GetMapping("file_upload_form2")
	public String form() {
	...
	@PostMapping("/save2")
	public String save2(@ModelAttribute("uploadForm") FileUploadForm uploadForm, 
		...
		if (!uploadForm.getFiles().isEmpty() && files != null && files.size() > 0) { // 파일(들) 유효성 점검
			...
			if (imgFileKinds.contains(tmpFileExt.trim().toUpperCase())) {
				...
				for (MultipartFile multipartFile : files) {
					...
					// 암호화된 파일명 적용 
					File outFileName = new File(imageUploadPath + fileNamingEnDecodingUtil.encodeFilename(fileName));					

					// 파일 저장소(E:/students/lsh/upload/image/)에 저장
					try (FileOutputStream fos = new FileOutputStream(outFileName)) {
						...
						// ex) E:/students/lsh/upload/image/thumbnail
